...
import { MotionPathPlugin } from "https://cdn.skypack.dev/gsap/MotionPathPlugin.js";
import {
Scene,
Object3D,
Color,
...
class App {
  ...
  loadMatCaps() {
    this.textureBox = new TextureLoader().load(
    "https://iondrimba.github.io/madcaps/public/matcaps/736655_D9D8D5_2F281F_B1AEAB.webp");
  ...
  setup() {
    ...
    this.meshes = {
      ...
      boxes: {
        size: 0.4,
        geometry: () => {
          const width = this.meshes.boxes.size,
          height = this.meshes.boxes.size / 4,
          depth = this.meshes.boxes.size;
    ...
    this.motion = {
      range: { inMin: 2.5, inMax: -2, min: 0.5, max: -2 },
      clamp: { min: -4.5, max: 0.05 },
      circular: {
        angle: 0,
        radius: 1 },
  ...
  addTweakPane() {
    ...
    this.motionGUI.addInput(this.motion.range, "inMin", {
      min: -100,
      max: 100,
      step: 0.1 });

    this.motionGUI.addInput(this.motion.range, "inMax", {
      min: -100,
      max: 100,
      step: 0.1 });

    this.motionGUI.addInput(this.motion.range, "min", {
      min: -100,
      max: 100,
      step: 0.1 });

    this.motionGUI.addInput(this.motion.range, "max", {
      min: -100,
      max: 100,
      step: 0.1 });
    ...
    on("change", env => {
      this.directionalLight.position.x = env.value;
    });

    this.guiLights.
    addInput(this.directionalLight.position, "y", { min: -100, max: 100 }).
    on("change", env => {
      this.directionalLight.position.y = env.value;
    });

    this.guiLights.
    addInput(this.directionalLight.position, "z", { min: -100, max: 100 }).
    on("change", env => {
      this.directionalLight.position.z = env.value;
    });
    ...
    this.guiColors.addInput(this.colors, "background").on("change", env => {
      this.scene.background = new Color(env.value);
      this.tweenColors(this.floor.material, hexToRgb(env.value));
    });

    this.guiColors.addInput(this.colors, "ball").on("change", env => {
      this.tweenColors(this.meshes.sphere.material, hexToRgb(env.value));
    });

    this.guiColors.addInput(this.colors, "grid").on("change", env => {
      this.tweenColors(this.grid.material, hexToRgb(env.value));
    });
  }
  ...
  addFloorGrid() {
    ...
    this.grid.position.set(0, -0.2, 0);
  ...
  addSphere() {
    const radius = this.meshes.sphere.size,
    width = 32,
    height = 32;
  ...
  addBlocksWall(boxWall, x, z) {
    ...
    for (let col = 0; col < cols; col++) {
      ...
      for (let row = 0; row < rows; row++) {
        ...
        front.position.set(x, 0.05, z);
        back.position.set(x, -0.05, z);
    ...
    for (let col = 0; col < cols; col++) {
      for (let row = 0; row < rows; row++) {
        if (x < 0) {
          ...
          dist = distance(
          x + y + 1,
          z - this.meshes.boxWall.container.position.z,
          front.position.x,
          front.position.z);
          ...
          dist = distance(
          x + y - 1,
          z - this.meshes.boxWall1.container.position.z,
          front.position.x,
          front.position.z);

        }

        const range = this.gsap.utils.mapRange(
        this.motion.range.inMin,
        this.motion.range.inMax,
        this.motion.range.min,
        this.motion.range.max,
        dist);

        const amount = this.gsap.utils.clamp(
        this.motion.clamp.min,
        this.motion.clamp.max,
        range);
  ...
  animateSphere() {
    ...
    const increaseAngle = () => this.motion.circular.angle -= 3;

    tl.to(this.meshes.sphere.mesh.position, {
      onUpdate: () => {
        this.meshes.sphere.mesh.position.x = -(
        Math.cos(radians(this.motion.circular.angle)) *
        this.motion.circular.radius);

        this.meshes.sphere.mesh.position.z =
        Math.sin(radians(this.motion.circular.angle)) *
        this.motion.circular.radius -
        Math.abs(this.motion.timeline.start);
        increaseAngle();
      },
      onComplete: () => {
